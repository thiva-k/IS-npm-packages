...
export interface AnimatedAvatarPropsInterface extends ImageProps, IdentifiableComponentInterface {
    ...
    name?: string;
    /**
     * Sets if the avatar should be gradient or not.
     */
    gradientAvatar?: boolean;
}
        ...
        name
            ? (
                <Image
                    rounded
                    centered
                    className={ gradientAvatar ? "animated-avatar gradient" : "animated-avatar" }
                    data-componentid={ `${ componentId }-with-initials` }
                    { ...rest }
                >
                    { !gradientAvatar && <AnimatedSVGBackground/> }
