...
import { AttributeMapping, Claim, TestableComponentInterface } from "@thiva/core/models";
import { CopyInputField } from "@thiva/react-components";
...
export const SummaryLocalClaims: FunctionComponent<SummaryLocalClaimsPropsInterface> = (
    ...
    return (
        <Grid className="wizard-summary" data-testid={ testId }>
            ...
            { data.claimURI ? generateSummaryLine(t("claims:local." +
                "wizard.summary.attributeURI"), showClaimURI()) : null }
            { data.displayOrder ? generateSummaryLine(t("claims:local." +
                "wizard.summary.displayOrder"), data.displayOrder) : null }
            { data.regEx ? generateSummaryLine(t("claims:local." +
