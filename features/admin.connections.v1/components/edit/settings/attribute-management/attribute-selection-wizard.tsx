...
interface AttributeSelectionWizardPropsInterface extends TestableComponentInterface {
    attributesList: ConnectionClaimInterface[];
    selectedAttributes: ConnectionCommonClaimMappingInterface[];
    setSelectedAttributes: (mapping: ConnectionCommonClaimMappingInterface[]) => void;
...
export const AttributeSelectionWizard: FunctionComponent<AttributeSelectionWizardPropsInterface> = (
    ...
    const [ tempAvailableClaims, setTempAvailableClaims ] = useState<ConnectionClaimInterface[]>([]);
    const [ filterTempAvailableClaims, setFilterTempAvailableClaims ] = useState<ConnectionClaimInterface[]>([]);

    const [ tempSelectedAttributes, setTempSelectedClaims ] = useState<ConnectionClaimInterface[]>([]);
    const [ filterTempSelectedClaims, setFilterTempSelectedClaims ] = useState<ConnectionClaimInterface[]>([]);

    const [ checkedUnassignedListItems, setCheckedUnassignedListItems ] = useState<ConnectionClaimInterface[]>([]);
    const [ checkedAssignedListItems, setCheckedAssignedListItems ] = useState<ConnectionClaimInterface[]>([]);
    ...
    const updateSelectedClaims = (() => {
        ...
        setSelectedAttributes(tempSelectedAttributes.map(element => {
            ...
            return isEmpty(existingMapping) ? {
                claim: element,
                mappedValue: ""
            } as ConnectionCommonClaimMappingInterface : existingMapping;
        }));
        setShowAddModal(false);
    });
        ...
        <Modal open={ showAddModal } size="small" className="user-attributes" data-testid={ `${ testId }-modal` }>
            ...
            <Modal.Content image data-testid={ `${ testId }-modal-content` }>
                ...
                >
                    ...
                    >
                        {
                            filterTempAvailableClaims?.map((claim: ConnectionClaimInterface) => {
                                return (
                                    <TransferListItem
