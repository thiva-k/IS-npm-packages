...
import { AlertLevels, RoleListInterface, RolesInterface, TestableComponentInterface } from "@thiva/core/models";
import { addAlert } from "@thiva/core/store";
import { Heading, Hint, Popup } from "@thiva/react-components";
...
export const OutboundProvisioningRoles: FunctionComponent<OutboundProvisioningRolesPropsInterface> = (
    ...
    const handleRoleAdd = (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        getRolesList(null)
            .then((response: AxiosResponse) => {
                if (response.status === 200) {
                    ...
                    setRoleList(allRole?.Resources?.filter((role: any) => {
                            ...
                            trigger={
                                (
                                    <Show when={ featureConfig?.identityProviders?.scopes?.update }>
                                        <Button
                                            onClick={ (e: React.MouseEvent<HTMLButtonElement>) => handleRoleAdd(e) }
                                            icon="add"
                                            type="button"
                                            disabled={ false }
                                            className="inline"
