...
/**
 * @fileOverview Script to copy theme content from @thiva/theme to source.
 *
...
const SOURCE_DIRECTORY = "./"; // Source code directory.
...
const themeModule = path.join(__dirname, "..", "node_modules", "@thiva", "theme", "dist", "lib", "themes");
const target = path.join(src, THEMES_DIRECTORY);
const imagesFolderRelPath = path.join(ASSETS_FOLDER_NAME, IMAGES_FOLDER_NAME);

log("\nStarted copying the theme images from @thiva/theme to the portal source.");

// Check if the `@thiva/theme` module is installed, if not terminate the script.
if (!fs.existsSync(themeModule)) {
    log(
        "\nCouldn't find @thiva/theme module." +
...
if (fs.existsSync(target)) {
    ...
    execSync("pnpm clean:themes:features");
}
